...
2. Update SystemC and UVM/SystemC Wrappers to Permit MSVC /Wall.
3. Allow Base Directory Override in Makefile.
4. Add project test-EXAMPLE-single_cycle.
5. Restrict Excessive Diagnostic Printout From test-EXAMPLE-single_cycle
...
1. There is a memory leak that was debugged and seems to be in the SystemC
   or UVM/SystemC references and not our transaction(s) and sequence item(s)
   Therefore, no plan to fix reference code. 
...
3. Under Linux the example the test [simple/phases/timeout/test] fails
